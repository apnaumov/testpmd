...
start_packet_forwarding(int with_tx_first)
{
	...
	if ((strcmp(cur_fwd_eng->fwd_mode_name, "rxonly") == 0 || strcmp(cur_fwd_eng->fwd_mode_name, "readpkt") == 0) && !nb_rxq)
		rte_exit(EXIT_FAILURE, "rxq are 0, cannot use rxonly fwd mode\n");

	if (strcmp(cur_fwd_eng->fwd_mode_name, "txonly") == 0 && !nb_txq)
		rte_exit(EXIT_FAILURE, "txq are 0, cannot use txonly fwd mode\n");

	if (((strcmp(cur_fwd_eng->fwd_mode_name, "rxonly") != 0 || strcmp(cur_fwd_eng->fwd_mode_name, "readpkt") != 0) &&
